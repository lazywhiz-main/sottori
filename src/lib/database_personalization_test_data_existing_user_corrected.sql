...
BEGIN
    ...
    -- 医療プロフィールの詳細設定（実際のフィールド名を使用）
    UPDATE user_medical_profiles 
    SET 
        cancer_type = 'breast_cancer',
        stage = 'stage_2',
        diagnosis_date = '2024-03-15'::date,
        current_treatment_types = ARRAY['chemotherapy', 'hormone_therapy'],
        treatment_status = 'ongoing',
        treatment_start_date = '2024-04-01'::date,
        treatment_history = jsonb_build_array(
            jsonb_build_object(
                'treatment', 'surgery',
        ...
        side_effects_experienced = ARRAY['fatigue', 'nausea'],
        primary_doctor = '田中先生',
        hospital = '都立病院',
        hospital_location = '東京都新宿区',
        updated_at = NOW()
    WHERE user_id = existing_user_id;

    -- コンテキストプロフィールの設定（実際のフィールド名を使用）
    ...
    SET 
        ...
        employment = 'working',
        family_support = 'high',
        mobility_limitations = ARRAY[]::text[],
