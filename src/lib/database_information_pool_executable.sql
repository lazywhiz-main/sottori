...
-- 検索ベクトル列を後から追加（日本語設定があることを前提）
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'information_pool' 
    AND column_name = 'search_vector'
  ) THEN
    ALTER TABLE information_pool ADD COLUMN search_vector tsvector 
    GENERATED ALWAYS AS (
      to_tsvector('simple', 
        COALESCE(title, '') || ' ' || 
        COALESCE(summary, '') || ' ' || 
